...
function state(E, n, l, Z) 
	if (length(epos(n,l,Z)) > 0 )
...
#	push!(myaxis, p)
	...
	#---------- 1s 
	for id = 1 :6 #zmax
		push!(myaxis, state(uni.s1,1, 0, id))
	...
	#---------- 2s 
	for id = 1 :zmax
		push!(myaxis, state(uni.s2,2, 0, id))
	...
	#---------- 2p
	for id = 1 :zmax
		push!(myaxis, state(uni.p2,2, 1, id))
		push!(myaxis, electrons(uni.p2, 2,1 , id))
	end

	#---------- 3s 
	for id = 1 : zmax
		push!(myaxis, state(uni.s3, 3,0, id))
	...
	#---------- 3p
	for id = 1 : zmax
		push!(myaxis, state(uni.p3, 3, 1, id))
		push!(myaxis, electrons(uni.p3, 3,1 , id))
	end

		#---------- 3d
	#for id = 1 : zmax
	#	push!(myaxis, state(uni.d3, 3, 2, id))
	...
	#---------- 4s
	for id = 1 : zmax
		push!(myaxis, state(uni.s4, 4, 0, id))
		push!(myaxis, electrons(uni.s4, 4,0 , id))
	end

	#---------- 4p
	for id = 1 : zmax
		push!(myaxis, state(uni.p4, 4, 1, id))
