...
# ╔═╡ 7949660d-d280-4429-a3f9-4798dea6f5b1
WF(x,y,z,phi,ML) = abs( (0.5 - ML .* 0.2 ).* psi_cart(x,y,z, QN(1,0,0))	+ psi_cart(x,y,z, QN(2,ML,0)) * exp(1im .* phi))^2


# ╔═╡ a92bc776-0071-4c3d-ba4c-f2121e17b2ab
function make_3d(WF, phi_step,ML)
	#qn = QN(3,2,0)

	dx2 = 5
	# calculate WF
	xs = range(-dx2,dx2; length=150)
	values =[ WF(x,y,z, 2*pi/8 .* (phi_step +4), ML)  for x =xs, y=xs, z=xs]
	...
	volume!(ax,values, algorithm = :iso, isorange = 0.001, isovalue = 0.004, 
		backlight=0.9, specular=0.9, shading=FastShading, colorrange = (0,0.005))
	
	#volume!(ax,values, algorithm = :iso, isorange = 0.05, isovalue = -0.5, backlight=0.9, specular=0.9, shading=FastShading)

	save("transition_$(ML)_$(phi_step).png", fig)
	return fig #, dx, dx2
end;

# ╔═╡ d7167706-831c-491f-bcf5-828bb87ec9d4
[make_3d(WF, phi,1) for phi in (0:7) ]
