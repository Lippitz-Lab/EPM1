...
# ╔═╡ 3976099a-25ab-4929-9539-29e32fa28279
# Define the potential function
function V_slope(x)
...
# ╔═╡ c12eea30-2209-472a-8dae-157c05e3ff6f
#V(x) = V_barrier(x);
V(x) = V_slope(x);
...
function schr!(du, u, p, x)
    du[1] = u[2]
    du[2] = (2 * (V(x) - E0) * u[1])  # second derivative as 2x first derivative
end;

# ╔═╡ a9da8a55-523a-437c-89fb-df3562d80c11
begin
	# Boundary conditions
	# wave function _after_ barrier
...
begin
	...
	plot(-x, real.(y ), label="real(Ψ)")
	plot!(-x, scale_energy .* V.(x), label ="U(x)")
	plot!(-x, scale_energy .* E0 .+ V.(x) .* 0, label="E", xlabel="position", ylabel="WF, energy")
end
...
manifest_format = "2.0"
project_hash = "f7881ffe32514982a289244bc026f0b3229a4616"
