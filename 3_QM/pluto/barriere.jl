...
# ╔═╡ 8ab011b8-93b2-11ef-21ef-e17eb048e31b
using DifferentialEquations

# ╔═╡ d3d16944-da64-4f78-9390-f402616cdd95
using PlutoUI

# ╔═╡ 3ec08165-16e1-4938-bf3b-099727f04e49
using Plots

# ╔═╡ e8baa3d2-f12a-4623-8eec-9a187829b295
using PGFPlotsX

# ╔═╡ ae4d48d0-1e98-48c9-93b6-c18b939076d9
begin
	V0 = 0.6
	E0 = 0.55
	k  = sqrt(2 * E0)  # check !!
...
# ╔═╡ 48bbca42-31df-475b-beee-c95997b5908e
...
# ╔═╡ 5e1b0b65-0de8-422e-8ec3-1b80fbf4f245
V(3)

# ╔═╡ 8bb988ae-b18b-4eb5-9711-567d709e2386
# Define the Schrödinger equation as a function
function schr!(du, u, p, x)
    du[1] = u[2]
    du[2] = (2 * (V(x) - E0) * u[1])
end

# ╔═╡ a9da8a55-523a-437c-89fb-df3562d80c11
begin
	# Boundary conditions
	# wave fucntion _after_ barrier
	u0 = [1.0, -1im * k]  # [Psi(xmin), Psi'(xmin)]
	
	# Set up the problem
	prob = ODEProblem(schr!, u0, (xl, xr))
	
	# Solve the problem with specified max step size
	#sol = solve(prob, Tsit5())
	sol = solve(prob , Tsit5(); reltol = 1e-8, abstol = 1e-8, d_discontinuities = [-4,4])
end;

# ╔═╡ cafee983-7624-4eae-b09d-181c7e52ca9e
u1(mysol, t) = mysol(t)[2]

# ╔═╡ 2b19c1bf-95b8-45ea-9bd8-fd07986d6b3c
begin
	x = (-15:0.01:15)
	y = [ u1(sol,x1) for x1 in x]
	y = y .* exp(-1im .* angle( y[end]))
	plot(-x, abs.(y ))
	plot!(-x, 6 .* V.(x))
	plot!(-x, 6 .* E0 .+ V.(x) .* 0)
end

# ╔═╡ 1eb52b55-e7ca-4345-a207-783d7450ac46
maximum(real.(y[end-2000:end])) ./ maximum(real.(y[1:2000]))

# ╔═╡ 4c8ca3b3-4b9a-4b9a-9e72-b6ed6551b878
maximum(abs.(y[end-2000:end])) ./ maximum(abs.(y[1:2000]))

# ╔═╡ e8169939-fe87-4ac5-87ae-ec694a8bec7e
angle(y[1]), angle(y[end])

# ╔═╡ 5c331968-5530-400d-aa72-e74b5ffe09c6
maximum(abs.(y[end-2000:end])) ./ maximum(abs.(y[1:2000]))

# ╔═╡ 961874f1-e8ce-40e3-b80a-e1d26501e3f0
begin
	#x = (xl:0.01:xr)
	#y = [ u1(sol,x1) for x1 in x]
	plot(-x, imag.(y))
...
# ╔═╡ f33fe8f1-b642-4ff3-9819-23f75df0996e
scale_energy = 9;

# ╔═╡ feda1d04-d2e0-4aed-a198-8ee5c477f951
x0 = x[findlast( V.(x) .> E0)]

# ╔═╡ 4e193973-3bf3-4b7c-a478-48fcfe75f516
let
	...
	font = "\\footnotesize",
	 #       xmin = -200, xmax =200,
			...
			xmajorticks="false",
			#ylabel = raw"Signal (V)",
		    #xlabel=raw"Position x",
			ytick =  [0 ], "axis on top=true",
		#"hide axis",
	}
		);

			push!(myaxis, "\\fill[gray!30!white] ( $(-x0),0) rectangle ( $(x0), 10);")
	...
	pgfsave("../barrier.tikz.tex",myaxis; include_preamble= false)
	myaxis
...
manifest_format = "2.0"
project_hash = "e101a347b7e3d237c13ad6974e74183ac6e7c761"
...
deps = ["ADTypes", "Adapt", "ArrayInterface", "DataStructures", "DiffEqBase", "DiffEqNoiseProcess", "DocStringExtensions", "FastPower", "FiniteDiff", "ForwardDiff", "JumpProcesses", "LevyArea", "LinearAlgebra", "Logging", "MuladdMacro", "NLsolve", "OrdinaryDiffEqCore", "OrdinaryDiffEqDifferentiation", "OrdinaryDiffEqNonlinearSolve", "Random", "RecursiveArrayTools", "Reexport", "SciMLBase", "SciMLOperators", "SparseArrays", "StaticArrays", "UnPack"]
git-tree-sha1 = "a7d5d87185450b61a95000547c85401ffd8e6e42"
...
git-tree-sha1 = "dc199915b7d2d1a25c8b66968e905f9cc671c1be"
