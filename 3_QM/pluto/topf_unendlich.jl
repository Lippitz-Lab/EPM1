	...
	for id = 1:4
	 ...
	 p = @pgf PGFPlotsX.Plot(
       ...
       Table(["x" => x |> ustrip, "y" =>  Es[id] .+ scale .* 1.5e-1 .* ψs[:,id] |> ustrip])
		...
		push!(myaxis, p)
		push!(myaxis, "\\node[below] at (0.8, $(Es[id]|>ustrip) ) 
		{$(round(Es[id]|>ustrip,digits=2)) eV} ;" )

	end

	
		push!(myaxis, "\\draw[thick, blue] (-0.5, 10) -- (-0.5,0) -- (0.5,0) -- (0.5,10);")

	
	pgfsave("../topf_unendlich.tikz.tex",myaxis; include_preamble= false)
	myaxis
...
manifest_format = "2.0"
project_hash = "edf98507439076baac69cdff7da556b35d16ea49"
