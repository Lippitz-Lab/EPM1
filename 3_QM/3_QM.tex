\renewcommand{\lastmod}{3. November 2025}
...
\begin{equation}
    \hat{H} = - \frac{\hbar^2}{2m} \frac{d^2}{dx^2} + U(x) \quad .
\end{equation}
...
\begin{equation}
     \frac{\hbar^2}{2m}  \left(\frac{2 \pi}{\lambda} \right)^2 \,  \Psi(x) + \left[ U(x) - E \right] \Psi(x) \overset{?}{=} 0 \quad .
...
\begin{marginfigure}
    ...
    \caption{Wenn das Potential $U(c)$ räumlich variiert, dann ändert sich die Wellenlänge der Wellenfunktion.}
...
% XXX im nächsten Durchgang hier weglassen und erst beim Harm. Oszillator diskutieren, dann braucht es das Ball-Beispiel nicht!
...
\begin{equation}
    WK(x, \delta x) = \frac{2 \delta t}{T } = \frac{2 \delta x}{T v(x)}
\end{equation} 
und die Wahrscheinlichkeitsdichte
\begin{equation}
    P(x) = \frac{WK(x, \delta x)}{\delta x} = \frac{2}{T v(x)} \quad .
\end{equation}
...
\begin{marginfigure}
    ...
    \caption{Tunneln durch eine Barriere. Dargestellt ist der Realteil (dünn) und der Betrag (dick) der Wellenfunktion $\Psi(x)$. Der grau unterlegte Bereich $U > E$ ist klassisch verboten.}
    \label{fig:3_barrier}
\end{marginfigure}
