...
# ╔═╡ ca132b18-a070-4274-b890-196183747233
begin
	xL = []
	yL = []
	zL = []
	cL = []
	while (length(xL) < 10000)
		(x,y,z) = rand((1:150),3)
		if (rand() < values[x,y,z].^2) #& (values[x,y,z] > 0)
			push!(xL, x)
			push!(yL, y)
			push!(zL, z)
				if (values[x,y,z] < 0)
...
# ╔═╡ d68fad15-ba52-4a49-b284-11c5c396a8d6
begin
		mm = 96/25.4
		pt = 4/3
		
		fig = Figure(size = (400mm, 400mm), fontsize = 8pt)
		ax = LScene(fig[1, 1], show_axis=false)
		
	
		scatter!(ax,xL, yL, zL,  color= cL, markersize = 10, # color = (:black, 0.5),
			rasterize = 50, overdraw = true )

	if (1==0)
		volume!(ax,values, algorithm = :iso, isorange = 0.05, isovalue = 0.5, 
				backlight=0.9, specular=0.9, shading=FastShading, 
			alpha=0.5, transparency=true )
			
		volume!(ax,values, algorithm = :iso, isorange = 0.05, isovalue = -0.5, backlight=0.9, specular=0.9, shading=FastShading, alpha=0.5, transparency=true )
	end
		
		save("scatter_WF.png", fig )
		fig  
	
end

# ╔═╡ 13ecf487-db2e-47b7-951d-e7f22b838568
let
		
			mm = 96/25.4
			pt = 4/3
			
			fig = Figure(size = (100mm, 100mm), fontsize = 8pt)
			ax = LScene(fig[1, 1], show_axis=false)
...
manifest_format = "2.0"
project_hash = "58cb02fb7fac29e2b4ab75839c2c3c5f4349a871"
