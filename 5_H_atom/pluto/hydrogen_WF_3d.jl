...
# ╔═╡ d1cc76e8-c7f0-4763-9a64-de33dcb57974
begin
	...
	p = Plot(	
		...
		#slices_x=attr(show=true, locations=[0],fill=1), # slice along x=0 plane
			showscale =false,
			spaceframe=attr(show=false),
			caps_x=attr(show=false),
			caps_y=attr(show=false),
			caps_z=attr(show=false),
	),
		Layout(width=600, height=400) # adjust size in pixel
	 )
end

# ╔═╡ 3a340d86-ac18-48b1-af04-717fad394db6
begin
	# this needs to be a capital P in Plot !
	Plot(	
		isosurface(
	    ...
	    isomin=-0.3,	# symmetric around 0
	    isomax=0.3,
			colorscale=colors.RdBu_3,
			
	#lighting_ambient=0.6,lighting_specular=0.9,
	#		opacityscale="uniform",
	    opacity=0.5, # needs to be small to see through all surfaces
	    surface_count=2, # even number
		#slices_x=attr(show=true, locations=[0],fill=1), # slice along x=0 plane
			showscale =false,
			spaceframe=attr(show=false),
			caps_x=attr(show=false),
		...
		scene_xaxis=attr(showticklabels=false, showbackground=false),
		scene_yaxis=attr(showticklabels=false, showbackground=false),
		scene_zaxis=attr(showticklabels=false, showbackground=false),
    
		) # adjust size in pixel
	 )
end

# ╔═╡ 75320955-63cd-4a86-8c50-038cfc9d36ec
PlotlyJS.savefig(p, "test.png")

# ╔═╡ 2700fa68-0d51-4722-ac11-567a73c3ecbc
values
...
manifest_format = "2.0"
project_hash = "7be3ef96b5b46d2c224fd754a8bacc8293a6a0d0"
