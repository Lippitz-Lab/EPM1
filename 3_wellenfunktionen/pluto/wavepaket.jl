...
# ╔═╡ 77b4fcc3-98d6-4f9b-8dc6-42310ff7378e
t = (-5:0.01:5)

# ╔═╡ 22f64696-e76c-4df8-83ed-23e1c29f730a
freqs = 1 .+ (-3:3) .* 0.13

# ╔═╡ 39e328e4-53a8-4295-ae67-9fb1c9046a77
amps = exp.(- (-3:3).^2 ./ 2.5.^2)

# ╔═╡ 36f0b19a-91c8-462a-9927-6e0fae6968cd
begin
	wp = sum(ones(size(t))* amps' .* cos.(t * freqs' .* 2 .* pi), dims=2) 
	plot(t, wp)
end
...
begin
	plot()
	offsets = zeros(size(freqs))
	for id = 1:length(freqs)
		offsets[id] = (sum(amps[1:id]) .+  sum(amps[1:id-1])) .* 1.5 
		plot!(t,  offsets[id] .+ amps[id] .* cos.(t .* freqs[id] .* 2 .* pi))
	end
	plot!()
end
...
let
	...
	for id = 1:length(freqs)
        ...
        Table([t,  
			offsets[id] .+ amps[id] .* cos.(t .* freqs[id] .* 2 .* pi) ])
    )
		push!(myaxis, p)
	 ...
	 p = @pgf PGFPlotsX.Plot(
       ...
       Table(["x" => t, "y" => -6 .+ wp])
		...
		push!(myaxis, raw"\draw[dashed] (0,-10) -- (0,14);")
		push!(myaxis, raw"\draw[->] (5.2,-2) -- (5.2,14);")

		push!(myaxis, "\\draw (5.1, $(offsets[4])) -- (5.3, $(offsets[4]))  ;")

		push!(myaxis, "\\node at (5.9, $(offsets[4])) {\$ f_0 \$ } ;")
		push!(myaxis, "\\node at (5.9, 14) {\$ f \$ } ;")
...
manifest_format = "2.0"
project_hash = "b166c05d93ba154bef3019dfbe8c03eccb05edae"
