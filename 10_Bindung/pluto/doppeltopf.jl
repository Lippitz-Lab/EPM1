...
md"""
Wir lösen die Schrödinger-Gleichung
```math
H \psi =  \left(- E_{kin} + U(x) \right) \psi =  \left( - \frac{\hbar^2}{2 m} \, \frac{\partial^2}{\partial x^2} + U(x)  \right) \psi = E \psi
```
Die zweifache Ableitung ist ungefähr 
```math
 \frac{\partial^2}{\partial x^2}  \psi  \approx \frac{\psi(x - \Delta x) - 2 \psi(x) +\psi(x + \Delta x)}{(\Delta x)^2}
```
Das Potential kann jede beliebige Form haben. Hier nehmen wir einen Potentialtopf
```math
...
function doppelkasten(x, d)
	# x Achse
	#x = (-5:0.01:5) .* 1u"nm"
	Δx = x[2] - x[1]   # Abstand der Gitterpunkte
	
	N = length(x)    # Anzahl Gitterpunkte

	# Matrix zur doppelten Ableitung
	Ekin = zeros(N,N)
	Ekin[diagind(Ekin,-1)] .= 1
	Ekin[diagind(Ekin,1)] .= 1
	Ekin[diagind(Ekin)] .= -2
	Ekin = Ekin .* ħ^2 ./(2 * m_e * Δx^2)
	...
	# 1d Potential
	Vx = zeros(N) .* 0u"eV" .+ V0
	Vx[ (-d/2-b .< x .< -d/2)] .= 0u"eV"
	Vx[ (d/2 .< x .< d/2 + b)] .= 0u"eV"

	# Kopieren in 2D Matrix
	V = zeros(N,N) .* 0u"eV"
	V[diagind(V)] .= Vx

	# alles addieren, Einheiten weglassen, weil sonst eigvec / eigvals nicht geht
	H = -Ekin  + V |> ustrip

	# Eigenwerte und -vektoren berechnen
	ψs = eigvecs(H)
    Es = eigvals(H) .* 1u"J" .|> u"eV" # Einheit wieder anfügen
	return Es, ψs , Vx
end;
...
let
	...
	Es, ψs , Vx = doppelkasten(x, 0.15u"nm")
	p = plot(x, Vx, 
		xaxis = ("position x", (-1.5, 1.5)), 
...
begin
		...
		Es1, ψs1 , Vx1 = doppelkasten(x, 0.15u"nm")
	#	Es2, ψs2 , Vx2 = doppelkasten(x, 0.3u"nm")
		Es3, ψs3 , Vx3 = doppelkasten(x, 0.8u"nm")
		Es4, ψs4 , Vx4 = doppelkasten(x, 1.3u"nm")
	...
	@pgf gp = GroupPlot(
		...
		height="55mm",
		   ...
		   { },
	 	[Plot( {red},
	       Table(["x" => x |> ustrip, "y" =>  Es1[id] .+ scale  .* ψs1[:,id] |> ustrip])) for id in 1:4],
		Plot(    {         thick, blue},
	 ...
	 { },
	 	[Plot( {red},
	       Table(["x" => x |> ustrip, "y" =>  Es3[id] .+ scale   .* ψs3[:,id] |> ustrip])) for id in 1:4],
		Plot(    {         thick, blue},
	       Table(["x" => x |> ustrip, "y" =>  Vx3 |> ustrip])
	    ),
...
# ╔═╡ 364c54d2-d9a7-4d3b-a4c9-4b67e1b925f2
# ╠═╡ disabled = true
#=╠═╡
	@pgf gp = GroupPlot(
	    {
	    ...
	    { ytick = "0", font = "\\footnotesize", ymax=8},
	    Plot({thick}, Table(x = r, y = P.(r; qn = (n = 1, m= 0, l=0)) )),
		"\\node at (3,5) {1s};",
		"\\draw[dotted] (1,0) -- (1,9);",
	
	    {ytick = "0",font = "\\footnotesize",ymax=3},
	    Plot({thick}, Table(x = r, y = P.(r; qn = (n = 2, m= 0, l=0)) )),
	    Plot({thick}, Table(x = r, y = P.(r; qn = (n = 2, m= 0, l=1)) )),
	   		"\\node at (8,2) {2s};",
	   		"\\node at (1.5,2) {2p};",
 		"\\draw[dotted] (4,0) -- (4,5);",

	    { ytick = "0", xlabel = raw" $r / a_B$  ", font = "\\footnotesize",ymax=1.6},
	    Plot({thick}, Table(x = r, y = P.(r; qn = (n = 3, m= 0, l=0)) )),
	    Plot({thick}, Table(x = r, y = P.(r; qn = (n = 3, m= 0, l=1)) )),
	    Plot({thick}, Table(x = r, y = P.(r; qn = (n = 3, m= 0, l=2)) )),
		   		"\\node at (5,1.1) {3d};",
	   		"\\node at (12,1.4) {3p};",
	   		"\\node at (17,1.1) {3s};",
  		"\\draw[dotted] (9,0) -- (9,5);",
...
# ╔═╡ 15d9c859-081d-4094-8dd7-7b28eb438d1c
# ╠═╡ disabled = true
#=╠═╡
let
...
manifest_format = "2.0"
project_hash = "fa5807cfb23d9a257b1f7f57f0ea30202cc46c3c"
