\renewcommand{\lastmod}{11. Oktober 2024}
...
\begin{equation}
   - \frac{\hbar}{2m} \frac{d^2}{dx^2} \Psi(x) + \left[ U(x) - E \right] \Psi(x) = 0
   \label{eq:4_SG_1d}
 \end{equation}
 Es handelt sich also um eine Differentialgleichung zweiter Ordnung, wie man es von einer Wellengleichung erwarten würde.


\paragraph*{Nebenbemerkung} In einer allgemeineren Form wird die Schrödingergleichung oft wie folgt geschrieben
\begin{equation}
    \hat{H} \, \Psi = E \, \Psi \quad .
    \label{eq:4_SG_op_1d}
\end{equation}
Dabei bezeichnet $\hat{H}$ den Hamiltonoperator. Ein Operator ist eine Rechenvorschrift, die auf eine Funktion (hier $\Psi$) wirkt und wieder eine Funktion liefert. Im obigen Fall ist der Hamilton-Operator also
\begin{equation}
...
Die Form Gl.   \ref{eq:4_SG_op_1d} heißt Eigenwertgleichung und $E$ heißt \emph{Eigenwert}, weil der Operator wieder die gleiche Funktion $\Psi(x)$ ergibt, nur multipliziert mit einem skalaren Wert $E$.
...
\begin{equation}
    \Psi(x) = \Psi_0 \, \sin\left( \frac{2 \pi x}{\lambda} \right)
...
\begin{equation}
    \frac{d^2}{dx^2} \Psi(x)  = - \left(\frac{2 \pi}{\lambda} \right)^2 \, \Psi(x)
\end{equation}
Eingesetzt in Gl.~\ref{eq:4_SG_1d} ergibt dies
\begin{equation}
     \frac{\hbar}{2m}  \left(\frac{2 \pi}{\lambda} \right)^2 \,  \Psi(x) + \left[ U(x) - E \right] \Psi(x) \overset{?}{=} 0
     \label{eq:4_Sg_plausi}
\end{equation}
Unser Teilchen ist hier ein freies Teilchen. Es wirken keine äußeren Kräfte. Das Potential $U(x) = 0$. Die Energie $E$ ist vollständig in der kinetischen Energie $K$ , also 
\begin{equation}
    E =  K = \frac{1}{2} \, m \,  v^2 =  \frac{1}{2m} \, \left( \frac{h}{\lambda} \right)^2
...
Was geschieht, wenn das Teilchen nicht frei ist? Die Kräfte, die dann auf das Teilchen wirken, werden durch das Potential $U(x)$ beschrieben. Die Gesamtenergie $E$ bleibt erhalten, so dass die kinetische Energie $K(x)$ ist.
\begin{equation}
    K(x) = E - U(x)
\end{equation}
Das ist genau der Term in der Klammer der Schrödingergleichung Gl \ref{eq:4_SG_1d}! Über Gl. \ref{eq:4_Ekin_lambda} gibt es einen Zusammenhang zwischen der kinetischen Energie und der Broglie-Wellenlänge. Wenn das Potential räumlich nicht konstant ist, ändert sich die de Broglie-Wellenlänge der Wellenfunktion. Wenn weniger Energie für die kinetische Energie 'übrig' bleibt, wird die Wellenlänge größer bzw. die Geschwindigkeit kleiner. Abbildung XXX zeigt ein Beispiel.

\begin{marginfigure}
    \caption{slope potential WF XXX}
\end{marginfigure}
...
\begin{marginfigure}
    \caption{XXX POtential Teilchen im Kasten}
\end{marginfigure}

Wir zeichnen das Potenzial (Abb. XXX). Wir schneiden die Potentialstufen, die ins Unendliche gehen würden, ab und ersetzen sie durch einen Pfeil nach oben.

Das Teilchen kann sich physikalisch nicht außerhalb des Kastens befinden. Daher muss die Wellenfunktion dort Null sein, d.h.\begin{equation}
    \Psi(x) = 0 \quad \text{für} \quad x < 0  \quad \text{oder} \quad x > L
 \end{equation}
...
\begin{equation}
    \frac{d^2}{dx^2} \Psi(x) = - \beta^2 \Psi(x) 
  \end{equation}
...
\begin{equation}
    \Psi(0) =  A \sin   0  \, + \, B \cos   0 = B \overset{!}{0} \quad \text{also} \quad B = 0
\end{equation}
Ebenso muss $\Psi$ bei $x=L$ Null sein, also 
\begin{equation}
    \Psi(L) = A \sin \beta L \overset{!}{0} \text{also} \quad \beta L = n \, \pi \quad \text{mit} \quad n = 1, 2, 3, \dots
\end{equation}
...
\begin{equation}
    \Psi_n(x) = A \sin \beta_n x = A \sin \left(  \frac{n\pi}{L} x \right) \quad \text{mit} \quad n = 1, 2, 3, \dots
\end{equation}
...
und so 
\begin{equation}
    E_n = \frac{\pi^2 \, \hbar^2}{2 m L^2} \, n^2 \quad \text{mit} \quad n = 1, 2, 3, \dots
    \label{eq:4_En_kasten}
\end{equation}
Damit haben wir die Quantisierung. Nur diese Energiewerte sind für ein Teilchen im Kasten möglich. Dies ist eine Folge des Kastens, des confinement, nicht eine Eigenschaft des Teilchens selbst.  Abb. XXX skizziert diese Leiter der Energie-Eigenwerte. In solchen Skizzen hat nur die vertikale Achse als Energieachse eine Bedeutung. Die horizontale Achse hat keine physikalische Bedeutung und dient nur der graphischen Darstellung.

\begin{marginfigure}
    \caption{XXX Energie-Eigenwerte Teilchen im Kasten}
\end{marginfigure}
...
also
\begin{equation}
    A_n = \sqrt{\frac{2}{L}} \quad \text{für alle }n
\end{equation}
...
Damit wissen wir für das Teilchen im Kasten, dass die möglichen diskreten Werte der Energie durch Gl.  \ref{eq:4_En_kasten}. Es gibt eine minimale Energie 
\begin{equation}
E_1 = \frac{\pi^2 \, \hbar^2}{2 m L^2}
\end{equation}
und alle anderen steigen quadratisch in $n$ an. Die Wellenfunktion ist eine stehende Welle, gegeben durch Gl. \ref{eq:4_psi_kasten}. Die Zahl $n$ beschreibt die Anzahl der Bäuche. An den Wänden der Box befinden sich immer Knoten.
Dies sind die stationären Zustände des Systems.

Die Wahrscheinlichkeitsdichte ist gegeben durch
\begin{equation}
    P_n(x) = |\Psi_n(x)|^2 = \frac{2}{L} \, \sin^2 \left(  \frac{n\pi}{L} x \right)
    \label{eq:4_WK_kasten}
\end{equation}
Abbildung XXX zeigt dies für die ersten Werte von $n$. Es gibt also Bereiche entlang des Ortes $x$, in denen die Wahrscheinlichkeit, das Teilchen zu finden, gleich Null ist. Und es gibt $n$ Bereiche, in denen die Wahrscheinlichkeit maximal wird.

Skizzen vom Typ XXX werden gerne verwendet, um Wellenfunktionen und Eigenenergien gleichzeitig darzustellen. Dabei ist jedoch Vorsicht geboten. Das Potential $U(x)$ ist leicht verständlich und wird 'wie üblich' gezeichnet. Die Eigenenergie $E_n$ hat keine $x$-Abhängigkeit. Nur die Lage der Linie auf der y-Achse ist von Bedeutung. Die Energie $E_n$ und das Potential $U(x)$ haben jedoch eine gemeinsame Energie-y-Achse. Die Wellenfunktion $\Psi_n(x)$ teilt sich mit dem Potential die x-Achse. Man verschiebt aber gerne den Nullpunkt von $\Psi_n(x)$ so, dass er auf $E_n$ liegt. Und natürlich hat $\Psi_n(x)$ eine andere Einheit als Energie. Es gibt also eine zweite Skala für die y-Achse, aber die zeichnet man eigentlich nie.
...
\begin{equation}
    E_1 = \frac{\pi^2 \, \hbar^2}{2 m L^2} > 0
\end{equation}
...
\begin{equation}
    \frac{1}{2} m v^2 = \frac{\pi^2 \, \hbar^2}{2 m L^2} 
\end{equation}
...
\begin{equation}
    P(x) = \frac{WK(x, \delta x)}{\delta x} = \frac{1}{2 T v(x)}
\end{equation}
...
\begin{equation}
    v(x) = \frac{2L}{T} \quad \text{und} \quad P(x) = \frac{1}{L}
\end{equation}
...
In der Quantenmechanik ist die Situation ähnlich.  Die Berechnung ist in den üblichen Lehrbüchern der Quantenmechanik enthalten. Hier diskutieren wir nur die Ergebnisse. Abbildung XXX zeigt ein Beispiel für einen Elektron in einem Kasten mit einer Breite von 2~nm und einer Tiefe von 1~eV, wie er für Elektronen in Halbleitern typisch ist.
\begin{itemize}\setlength{\itemsep}{0pt}
    ...
    \item Der Hauptunterschied besteht darin, dass nun die Wellenfunktionen und damit auch die Wahrscheinlichkeitsdichte in den klassisch verbotenen Bereich hineinreichen. Das Teilchen wird an dieser niedrigen Wand reflektiert, dringt aber teilweise in die Wand ein.
\end{itemize}
...
\begin{equation}
    \eta = \frac{\hbar}{\sqrt{2m ( U_0 - E)}} \quad \text{für} \quad U_0 > E
\end{equation}
Nach dieser Länge $\eta$ ist die Wellenfunktion in der Wand auf $e^{-1} \approx 0,37$ abgefallen. Für makroskopische Teilchen ist die Eindringtiefe oder Abfall-Länge $\eta$ vernachlässigbar, nicht aber für Quantenobjekte. Diese sind etwas 'unscharf' und werden nicht direkt an der Wand reflektiert, sondern etwas außerhalb.
...
Ein klassisches Teilchen schwingt mit der Kreisfrequenz 
\begin{equation}
    \omega = \sqrt{\frac{k}{m}}
\end{equation}
...
\begin{marginfigure}
    ...
    \caption{Wellenfunktionen und Wahrscheinlichkeitsdichte im harmonische Oscillator}
\end{marginfigure}

Die zugehörigen Eigenenergien $E_i$ sind
\begin{equation}
    E_n = \left( n - \frac{1}{2}\right) \, \hbar \omega \quad \text{mit} \quad n = 1, 2, 3, \dots
\end{equation}
...
Auch hier gilt das Korrespondenzprinzip. Für große Quantenzahlen geht die Bewegung des quantenmechanischen harmonischen Oszillators in die klassische Bewegung über. Die Skizze XXX zeigt die Wahrscheinlichkeitsdichte für den Zustand $n=11$ im Vergleich zum klassischen Fall. Gut zu erkennen ist die erhöhte Aufenthaltswahrscheinlichkeit an den Umkehrpunkten der Bewegung, an denen die Geschwindigkeit gering ist.
