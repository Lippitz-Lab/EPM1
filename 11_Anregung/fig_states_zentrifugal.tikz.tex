...
%	\draw (-1.5,-2.5) rectangle (3.5,7.5);
   ...
   \tikzmath{\x1 = 0; \x2 = 2.5; \bsxx = 0.12;  \ylu = -1.7; \ylo = -1.3; \arrowoffs = 0.23; \zf = 0.002;}
% %  \tikzmath{\x1 = 0; \x2 = 2.4; \bsxx = 0.16;  \ylu = -1.7; \ylo = -1; \arrowoffs = 0.3;}

 % Zustände
  \foreach \a in {0,...,7}
  {
  		\tikzmath{\b = \a * (\a + 1) * \bsxx;}
  		\tikzmath{\c =  \a * (\a + 1) * \bsxx - \bsxx * \zf * \a^2 * (\a + 1)^2;}
...
%  % Übergangs-Pfeile
  \foreach \a in {0,...,6}
  {
 		\tikzmath{\b1 = (\a * (\a + 1)) * \bsxx;  \b2 = (\a +2)* (\a + 1) * \bsxx; \xa = \x1 + (\a +1) * \arrowoffs; \xn = \b2 - \b1;} 	
  ...
  \tikzmath{\bsxx = 1.7 * \bsxx;}
  
  % linien
   \foreach \a in {1,...,6}
   {
  		\tikzmath{\xn =  2 * \a * \bsxx ;
		\yn =  		2 * \a * \bsxx  + \bsxx * \zf * \a^2 * (\a + 1)^2 -  \bsxx * \zf * (\a + 2)^2 * (\a + 1)^2;
  		} 	
