...
% %	\draw (-1.5,-2.5) rectangle (3.5,7.5);
  \tikzmath{\x1 = 0;  \dx1 = 3;  \dx2 = 2.5; \wv = 2;  \brot = 0.05;    \ylu = -1.7; \ylo = -1; \arrowoffs = 0.12; }
 ...
 % Zustände
  \foreach \vib in {0,...,1}
  {
    	...
    	\draw[line width=1pt] (\x1,\b) -- ++ (\dx1,0) node[anchor=west]{\vib};
    	  ...
    	  \foreach \rot in {1,...,4}
     {
    	 		\tikzmath{\brxx1 = \b + \brot * \rot * (\rot +1);  }
  	\draw[line width=0.5pt] (\x1,\brxx1) -- ++ (\dx2,0) node[anchor=west]{\footnotesize \rot};
  ...
  % Übergangs-Pfeile
   \foreach \a in {1,...,4}
   {
  		\tikzmath{\xa1 = \x1 + (\a +1) * \arrowoffs;} 		 
  		 \tikzmath{\brxx1 = \wv * (0 + 0.5) + \brot * \a * (\a +1);  
  				 \brxx2 = \wv * (1 + 0.5) + \brot * \a * (\a - 1);  }
    	\draw[->] (\xa1,\brxx1) -- node (P\a) {}  (\xa1,\brxx2) ;
  }
    ...
    \foreach \a in {0,...,4}
   {
  		\tikzmath{\xa1 =0.8 +  \x1 + (\a +1) * \arrowoffs;} 		 
  		 \tikzmath{\brxx1 = \wv * (0 + 0.5) + \brot * \a * (\a +1);  
  				 \brxx2 = \wv * (1 + 0.5) + \brot * \a * (\a + 1);  }
    	\draw[->] (\xa1,\brxx1) -- node (Q\a) {}  (\xa1,\brxx2) ;
  }
    ...
    \foreach \a in {0,...,3}
   {
  		\tikzmath{\xa1 =1.6 +  \x1 + (\a +1) * \arrowoffs;} 		 
  		 \tikzmath{\brxx1 = \wv * (0 + 0.5) + \brot * \a * (\a +1);  
  				 \brxx2 = \wv * (1 + 0.5) + \brot * (\a+1) * (\a + 2);  }
    	\draw[->] (\xa1,\brxx1) -- node (R\a) {}  (\xa1,\brxx2) ;
  }
