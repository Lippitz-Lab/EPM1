
\begin{tikzpicture}
\useasboundingbox (-1.3,-1.2) rectangle (10.2,4.7);
%\draw (-1.3,-1.2) rectangle (11.2,4.7);

    \begin{axis}[no markers,  xlabel={Wellenzahl $\bar{\nu}$ (cm$^{-1}$)}, ylabel={Dielektrische Funktion $\epsilon'$, $\epsilon''$},  width=115mm, height=60mm, xmode=log, xmin = 6e-2, xmax= 2e5, unbounded coords=jump, ymin=0, ymax = 4
    ]
    \addplot table [x index=0, y index=1] {\currfiledir data/water.dat};
    \addplot table [x index=0, y index=2] {\currfiledir data/water.dat};
	
     \node[anchor=north east]  at (axis cs:30,4){x20};
	\draw [dashed] (axis cs:30,0) -- (axis cs: 30,4);
	
	     \node[anchor= west]  at (axis cs:0.4,3){ $\epsilon'$};
     \node[anchor= west]  at (axis cs:3,1){ $\epsilon''$};
   
     \node[anchor= west]  at (axis cs:150,3){ $\epsilon'$};
     \node[anchor= west]  at (axis cs:200,0.6){ $\epsilon''$};

    \end{axis}
\end{tikzpicture}

